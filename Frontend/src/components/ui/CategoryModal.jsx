...
import { addCategoryAsync, updateCategoryAsync, selectCategoryStatus } from '../../redux/slices/categorySlice';
...
const CategoryModal = ({ isOpen, onClose, initialCategory = {} }) => {
  ...
  const handleSubmit = () => {
    if (categoryName) {
      if (isEditing) {
        dispatch(updateCategoryAsync({ id: initialCategory._id, data: { name: categoryName } }));
      } else {
