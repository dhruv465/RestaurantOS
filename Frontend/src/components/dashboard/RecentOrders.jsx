...
const RecentOrders = () => {
  ...
  const tableStatusUpdateMutation = useMutation({
    mutationFn: ({ tableId, status, orderId }) => updateTable({ tableId, status, orderId }),
    onSuccess: (data) => {
                ...
                <tr key={index} className="border-b border-gray-600 text-[var(--text-color)] cursor-pointer">
                  ...
                  <td className="p-4">
                    <select
                      ...
                      onChange={(e) => handleOrderStatusChange({ orderId: order._id, orderStatus: e.target.value })}
                    >
                  ...
                  <td className="p-4">
                    <select
                      className={`bg-[#1a1a1a] text-[#f5f5f5] border border-gray-500 p-2 rounded-lg focus:outline-none cursor-pointer ${
                        order.table.status === "Available" ? "text-green-500" : "text-red-500"
                      }`}
                      value={order.table.status}
                      onChange={(e) => handleTableStatusChange({ tableId: order.table._id, tableStatus: e.target.value, orderId: order._id })}
                    >
